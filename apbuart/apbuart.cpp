...
//Default Constructor
apbuart::apbuart(sc_module_name module_name, uint32_t socket_port): sc_module( module_name ),
                                        target_export("iport"),
...
#ifndef UART_debug
      dbg_printf("Serial unit read accessed at: %#x, with value = %#x \n", request.addr, request.data);
#endif
         return response;
    default:
         response.status = ERROR;
#ifndef UART_debug
      dbg_printf("Serial unit wrongly accessed at: %#x, with value = %#x \n", request.addr, request.data);
#endif
         return response;
    case LOCK:break;
    case UNLOCK:break;
    case REQUEST_COUNT:break;
  }
}
